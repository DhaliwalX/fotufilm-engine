...
export async function measureTone(
  ...
  const gw = Math.min(
    width,
    Math.max(1, Math.floor((width * 64 + Math.floor(long / 2)) / long)),
  )
  const gh = Math.min(
    height,
    Math.max(1, Math.floor((height * 64 + Math.floor(long / 2)) / long)),
  )
  ...
  for (let top = 0; top < height; top += 32) {
    if (signal?.aborted)
      throw new DOMException('Tone measurement cancelled.', 'AbortError')
    const rows = Math.min(32, height - top)
    const pixels = decode(await source.read(0, top, width, rows))
    for (let y = 0; y < rows; y++)
      for (let x = 0; x < width; x++) {
        const i = (y * width + x) * 4,
          cell =
            Math.floor(((top + y) * gh) / height) * gw +
            Math.floor((x * gw) / width)
        const metered =
...
export function toneKey(grid, stops, x, y, width, height) {
  ...
  const gx = Math.min(
    Math.max(((x + 0.5) * grid.width) / width - 0.5, 0),
    grid.width - 1,
  )
  const gy = Math.min(
    Math.max(((y + 0.5) * grid.height) / height - 0.5, 0),
    grid.height - 1,
  )
  ...
  const sample = (plane) =>
    (1 - fy) *
      ((1 - fx) * plane[y0 * grid.width + x0] +
        fx * plane[y0 * grid.width + x1]) +
    fy *
      ((1 - fx) * plane[y1 * grid.width + x0] +
        fx * plane[y1 * grid.width + x1])
