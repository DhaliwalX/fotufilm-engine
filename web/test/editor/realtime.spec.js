...
test("Halide Normal agrees with the reference across all light and grade controls", async ({
  ...
  const report = await page.evaluate(async () => {
    const { createNormalDeveloper, developNormalReference, pixelSource } =
      await import("/src/engine.js");
    const { defaultEdit } = await import("/src/editor-state.js");
    const developer = await createNormalDeveloper();
    if (!developer)
      throw new Error("Build the Halide Normal kernel before testing.");
    const width = 240,
      height = 160,
...
for (const stock of [null, "Gold 200"])
  test(`continuous edits publish frames before release: ${stock || "Normal"}`, async ({
    page,
  }) => {
    await page.goto("/");
    ...
    await page.evaluate(() => {
      ...
      new MutationObserver(() =>
        window.previewFrames.push(performance.now()),
      ).observe(document.querySelector(".photo-plane img"), {
        attributes: true,
        attributeFilter: ["src"],
      });
    });
    const drag = async (name) => {
      ...
      console.log(
        `${stock || "Normal"} ${name} frames during drag:`,
        frames.length,
      );
      expect(frames.length).toBeGreaterThanOrEqual(8);
      const duringSize = await page.locator(".photo-plane").boundingBox();
      expect(Math.abs(duringSize.width - beforeSize.width)).toBeLessThanOrEqual(
        2,
      );
      await page.mouse.up();
      await expect(
