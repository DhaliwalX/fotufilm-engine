...
test('print encoding matches native dithered quantization without a second rounding', async () => {
  ...
  const result = await WebgpuDeveloper.prototype.develop.call(
    developer,
    source,
    {},
  )
...
test('packed GPU delivery copies tile interiors into a viewport for both bit depths', async () => {
  const { copyEncodedTile } = await import('../../src/output-encoding.js')
  const tile = {
    x: 12,
    y: 21,
    width: 2,
    height: 2,
    region: { x: 11, y: 20, width: 4, height: 4 },
  }
  const frame = { x: 10, y: 20, width: 5, height: 4 }
  for (const Type of [Uint8ClampedArray, Uint16Array]) {
    const encoded = Type.from({ length: 4 * 4 * 4 }, (_, i) => i + 1)
    const destination = new Type(frame.width * frame.height * 4)
    copyEncodedTile(destination, encoded, tile, frame)
    const expected = new Type(destination.length)
    expected.set(encoded.subarray(20, 28), 28)
    expected.set(encoded.subarray(36, 44), 48)
