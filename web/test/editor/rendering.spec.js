...
test("WebGPU and CPU agree with white balance, regional masks and grading", async ({
  page,
}) => {
  test.setTimeout(240000);
  await page.goto("/");
  const report = await page.evaluate(async () => {
    const { loadPack, createDeveloper, SimdDeveloper, pixelSource } =
    ...
    try {
      ...
      const rerolled = await gpu.develop(source, {
        ...controls,
        grain: 1,
        seed: 18,
      });
...
test("four crop corners move independently; rotate and flip undo cleanly", async ({
  page,
}) => {
  await page.goto("/");
...
test("export retains original dimensions above the preview limit", async ({
  page,
}) => {
  await page.goto("/");
