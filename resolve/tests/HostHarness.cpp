...
int testPlugin() {
    ...
    check(plugin->pluginVersionMajor == FOTUFILM_VERSION_MAJOR &&
          ...
          "carries the version project.yml names");
#endif
    ...
    // Activation. The harness runs with the licence bypassed; without it, a Mac that holds no
    // activation refuses the engine, and the refusal has to reach the user three ways — the
    // stock menu, the status line, and one message on the first render — and then heal on the
    // next node once the app is activated, without a relaunch. Driven only where the machine
    // is not activated: an activation on this Mac would make the engine come up regardless, and
    // the harness will not touch the user's certificate to find out otherwise.
    std::printf("activation\n");
    {
        const char *bypass = std::getenv("FOTUFILM_LICENSE_TEST_BYPASS");
        const std::string restore = bypass ? bypass : "";
        unsetenv("FOTUFILM_LICENSE_TEST_BYPASS");
        check(plugin->mainEntry(kOfxActionLoad, nullptr, nullptr, nullptr) == kOfxStatOK,
              "loads without an activation");
        if (fotufilm_bridge_stock_count() > 0) {
            std::printf("       this Mac holds an activation; the inactive path cannot be "
                        "driven here\n");
        } else {
            Effect describerHeld;
            describerHeld.properties = newPropertySet();
            plugin->mainEntry(kOfxActionDescribe,
                              reinterpret_cast<OfxImageEffectHandle>(&describerHeld),
                              nullptr, nullptr);
            Effect held;
            held.properties = newPropertySet();
            auto heldHandle = reinterpret_cast<OfxImageEffectHandle>(&held);
            check(plugin->mainEntry(kOfxImageEffectActionDescribeInContext, heldHandle,
                                    nullptr, nullptr) == kOfxStatOK,
                  "describes in context");
            std::string placeholder;
            {
                auto found = held.params.params.find("stock");
                if (found != held.params.params.end()) {
                    auto &strings = found->second->properties->strings;
                    auto options = strings.find(kOfxParamPropChoiceOption);
                    if (options != strings.end() && !options->second.empty()) {
                        placeholder = options->second[0];
                    }
                }
            }
            check(optionCount(held.params, "stock") == 1 &&
                      placeholder.find("not activated") != std::string::npos,
                  "the stock menu's one entry says the plugin is not activated");
            applyDefaults(held.params);
            check(plugin->mainEntry(kOfxActionCreateInstance, heldHandle, nullptr, nullptr) ==
                      kOfxStatOK, "creates an instance");
            check(getString(held.params, "stageStatus").find("not activated") !=
                      std::string::npos,
                  "and the status line says so");

            const int heldWidth = 32, heldHeight = 24;
            Clip *heldSource = held.clips[kOfxImageEffectSimpleSourceClipName].get();
            Clip *heldOutput = held.clips[kOfxImageEffectOutputClipName].get();
            for (Clip *clip : {heldSource, heldOutput}) {
                clip->pixels.assign(static_cast<size_t>(heldWidth) * heldHeight * 4, 0.25f);
                clip->image = newPropertySet();
                propSetPointer(handleOf(clip->image), kOfxImagePropData, 0,
                               clip->pixels.data());
                const int bounds[4] = {0, 0, heldWidth, heldHeight};
                propSetIntN(handleOf(clip->image), kOfxImagePropBounds, 4, bounds);
                propSetInt(handleOf(clip->image), kOfxImagePropRowBytes, 0,
                           heldWidth * 4 * static_cast<int>(sizeof(float)));
                propSetString(handleOf(clip->image), kOfxImageEffectPropPixelDepth, 0,
                              kOfxBitDepthFloat);
                propSetString(handleOf(clip->image), kOfxImageEffectPropComponents, 0,
                              kOfxImageComponentRGBA);
            ...
            PropertySet *heldArgs = newPropertySet();
            propSetDouble(handleOf(heldArgs), kOfxPropTime, 0, 0);
            const int heldWindow[4] = {0, 0, heldWidth, heldHeight};
            propSetIntN(handleOf(heldArgs), kOfxImageEffectPropRenderWindow, 4, heldWindow);
            setParam(held.params, "colorSpace",
                     static_cast<double>(fotufilm::Encoding::LinearDisplayP3));

            const int before = gMessagesPosted;
            check(plugin->mainEntry(kOfxImageEffectActionRender, heldHandle,
                                    handleOf(heldArgs), nullptr) == kOfxStatFailed,
                  "the first render fails");
            check(gMessagesPosted == before + 1 && gLastMessageType == kOfxMessageError &&
                      gLastMessageText.find("not activated") != std::string::npos,
                  "and tells the host, once, that the plugin is not activated");
            plugin->mainEntry(kOfxImageEffectActionRender, heldHandle, handleOf(heldArgs),
                              nullptr);
            check(gMessagesPosted == before + 1, "and the next frame says nothing more");

            // The user activates the app and adds a node. Two things have to happen, and one of
            // them is that nothing happens yet.
            setenv("FOTUFILM_LICENSE_TEST_BYPASS", "1", 1);

            // A second node, created the instant the activation lands. It does not reopen the
            // packs: `createInstance` throttles its retry the way the FxPlug side throttles its
            // own. Opening a project full of Fotufilm nodes creates them in a burst, and a
            // licence that was inactive a millisecond ago is inactive still — so a retry per node
            // would reopen the sealed packs once per node and refill, as it went, every menu
            // vector the render threads read without a lock. The wait is the plugin's
            // `kEngineRetrySeconds`, spelled again here because it is not exported; a build that
            // shortens it shortens this too.
            Effect late;
            late.properties = newPropertySet();
            auto lateHandle = reinterpret_cast<OfxImageEffectHandle>(&late);
            check(plugin->mainEntry(kOfxImageEffectActionDescribeInContext, lateHandle,
                                    nullptr, nullptr) == kOfxStatOK,
                  "describes a second panel, still with no engine behind it");
            applyDefaults(late.params);
            gAllowUndefinedParams = true;
            check(plugin->mainEntry(kOfxActionCreateInstance, lateHandle, nullptr, nullptr) ==
                      kOfxStatOK, "creates a node the instant the app is activated");
            gAllowUndefinedParams = false;
            check(getString(late.params, "stageStatus").find("cannot render") !=
                      std::string::npos,
                  "which does not reopen the packs: the retry is throttled, not one per node");

            std::this_thread::sleep_for(std::chrono::milliseconds(3100));
            gAllowUndefinedParams = true;
            check(plugin->mainEntry(kOfxActionCreateInstance, lateHandle, nullptr, nullptr) ==
                      kOfxStatOK, "creates another node once the interval has passed");
            gAllowUndefinedParams = false;
            check(getString(late.params, "stageStatus").find("cannot render") ==
                      std::string::npos,
                  "and that one finds the engine, and brings it up for every node in the host");

            // Which leaves the first node in the state nothing else in this harness can reach.
            // Its panel was built with no engine behind it, so it carries no texture toggle at
            // all, and its Texture Only selection reads back as zero — the one selection that
            // *is* the identity. Whether that zero means "the user selected nothing" or "there
            // was nothing to select" could not be decided when this node was built: the stage
            // list was empty then too, so the two looked alike. The node above filled that list
            // and nobody has re-created this one. Decided against the list as it stands, the
            // answer is that the toggles are missing, and the honest response is a refusal.
            char lateTexture[256] = "";
            if (fotufilm_bridge_stage_id(FOTUFILM_BRIDGE_STAGE_TEXTURE, lateTexture,
                                        sizeof(lateTexture)) > 0) {
                setString(held.params, "stageID", lateTexture);
                PropertySet *heldAnswer = newPropertySet();
                check(plugin->mainEntry(kOfxImageEffectActionIsIdentity, heldHandle,
                                        handleOf(heldArgs), handleOf(heldAnswer)) ==
                              kOfxStatReplyDefault &&
                          heldAnswer->strings.count(kOfxPropName) == 0,
                      "a node built before the engine came up is not the identity in Texture "
                      "Only, whatever its unreadable selection reads back as");
                std::fill(heldOutput->pixels.begin(), heldOutput->pixels.end(), -7.0f);
                check(plugin->mainEntry(kOfxImageEffectActionRender, heldHandle,
                                        handleOf(heldArgs), nullptr) == kOfxStatFailed,
                      "and fails the frame rather than passing the source through");
                bool heldUntouched = true;
                for (float value : heldOutput->pixels) {
                    if (value != -7.0f) heldUntouched = false;
                }
                check(heldUntouched, "having written nothing");
                std::fill(heldOutput->pixels.begin(), heldOutput->pixels.end(), 0.25f);
                setString(held.params, "stageID", "full");
                setParam(held.params, "stage", FOTUFILM_BRIDGE_STAGE_FULL);
            }
            plugin->mainEntry(kOfxActionDestroyInstance, lateHandle, nullptr, nullptr);

            check(plugin->mainEntry(kOfxActionCreateInstance, heldHandle, nullptr, nullptr) ==
                      kOfxStatOK, "creates an instance after activation");
            check(getString(held.params, "stageStatus").find("cannot render") ==
                      std::string::npos,
                  "and the status line no longer says it cannot render");
            check(plugin->mainEntry(kOfxImageEffectActionRender, heldHandle,
                                    handleOf(heldArgs), nullptr) == kOfxStatOK,
                  "and the node renders without a relaunch");
            bool wrote = false;
            for (float value : heldOutput->pixels) {
                if (value != 0.25f) wrote = true;
            }
            check(wrote, "writing a developed frame");
            plugin->mainEntry(kOfxActionDestroyInstance, heldHandle, nullptr, nullptr);
        }
        if (bypass) setenv("FOTUFILM_LICENSE_TEST_BYPASS", restore.c_str(), 1);
        check(plugin->mainEntry(kOfxActionUnload, nullptr, nullptr, nullptr) == kOfxStatOK,
              "unloads");
    }

    // The other half of that story: the node the host built *before* the engine came up. The
