// Generated by tools/calibration/h12383/extract_paper.py.
...
/// grid at 5 nm steps.
///
/// The only print medium here that is not a reflection paper: 2383 is
...
enum Vision2383PrintSpectra {
    ...
    /// The green record of the sheet's SENSITOMETRIC CURVES, fitted
    /// to the engine's softplus H&D family at RMS 0.0180 D.
    ///
    /// Green because the engine gives the print stage one
    /// characteristic curve and corrects the other two channels
    /// against green in `printingContrastScale` — the same reading
    /// E-7020's paper curve ships under.
    ///
    /// The x axis is the sheet's own log exposure in lux-seconds and
    /// nothing is subtracted from it, because nothing reads it: the
    /// engine anchors the print where the curve reaches
    /// `paperMidDensity` above base and works in stops from there, so
    /// only the shape is used. What is used, and what a paper cannot
    /// supply, is the range — 4.05 D against an RA-4 sheet's 2.1.
    static let printCurve = CharacteristicCurve(
        dMin: 0.0599568, gamma: 6.40924, toe: 0.652559, toeWidth: 0.215835, shoulder: 1.28499, shoulderWidth: 0.20276)
}
