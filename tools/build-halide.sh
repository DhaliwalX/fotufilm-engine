...
if [[ "${1:-}" == "--webgpu" ]]; then
  ...
  for patch in halide-webgpu-storage-limit halide-webgpu-pipeline-cache halide-webgpu-release-handles halide-webgpu-strict-float halide-webgpu-pow-zero halide-webgpu-error-waits halide-webgpu-buffer-cache halide-webgpu-frame-throughput halide-webgpu-kernel-modules halide-vulkan-strict-float halide-vulkan-bool-uniforms halide-vulkan-allocation-padding; do
